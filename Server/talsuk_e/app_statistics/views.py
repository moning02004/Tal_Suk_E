...
def add(request):
    try:
        ...
        day = Day(month=month, num=today.day, weight=0, fee=0) if not today.day in [x.num for x in month.day_set.all()] else month.day_set.get(num=today.day)
        day.weight = day.weight + int(data['weight'])
        day.fee = float(day.weight * user.suke.fee * 0.001)
        day.save()
